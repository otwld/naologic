...
export default class MarkerTool {
   ...
   */
  constructor({api}: any) {
    console.info('[MarkerTool] constructor', api);
    this.api = api;
  ...
  surround(range: any) {
    ...
    const termWrapper = this.api.selection.findParentTag(this.tag, MarkerTool.CSS);
   ...
   */
  checkState() {
    const termTag = this.api.selection.findParentTag(this.tag, MarkerTool.CSS);
